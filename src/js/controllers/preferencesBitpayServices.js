...
angular.module('copayApp.controllers').controller('preferencesBitpayServicesController',
  function($rootScope, $scope, $state, $timeout, $ionicHistory, bitpayAccountService, bitpayCardService, popupService, gettextCatalog) {
    ...
    $scope.removeAccount = function(account) {
      var title = gettextCatalog.getString('Remove FolmPay Account?');
      var msg = gettextCatalog.getString('Removing your FolmPay account will remove all associated FolmPay account data from this device. Are you sure you would like to remove your FolmPay Account ({{email}}) from this device?', {
        email: account.email
      });
    ...
    $scope.removeCard = function(card) {
      var title = gettextCatalog.getString('Remove FolmPay Card?');
      var msg = gettextCatalog.getString('Are you sure you would like to remove your FolmPay Card ({{lastFourDigits}}) from this device?', {
